...
const Hero = () => {
  ...
  return (
    <header className="pt-24 container mx-auto relative lg:pt-0 ">
      {/* ================= MOBILE VERSION ================= */}
      <div className="flex flex-col items-center gap-8 lg:hidden">
        ...
        {/* 2) รูป + การ์ด REAL-TIME + F (ใช้ดีไซน์เดิม) */}
        <div className="flex justify-center w-full">
          <div className="relative w-full max-w-[430px] h-[420px]">
            {/* BACK IMAGE */}
            <img
              src="/assets/img/bg-community-1.avif"
              className="absolute right-10 top-10 rotate-3 w-[210px] h-[200px] rounded-2xl shadow-xl shadow-black/70 object-cover"
              alt="Community"
            />
            {/* โลโก้ F */}
            ...
            {/* FLOATING CARD */}
            <div className="absolute right-[150px] top-[150px] rotate-[-5deg] bg-[#0d1125] rounded-2xl p-6 w-[210px] h-[220px] shadow-xl shadow-black/70 flex flex-col">
        ...
        {/* 4) Description */}
        <p className="max-w-[300px] text-center opacity-80 text-sm">
        ...
        {/* 5) ปุ่ม column ยาว 2 อัน */}
        <div className="flex flex-col gap-3 w-full max-w-[360px]">
          <Link
            href="/report/new"
            className="w-full px-6 py-3 rounded-full bg-linear-to-r from-orange-500 via-pink-500 to-fuchsia-500 text-sm font-semibold text-white shadow-xl shadow-orange-500/25 transition hover:brightness-110 cursor-pointer text-center"
          >
            Report Issue →
          </Link>

          <button className="w-full px-6 py-3 mt-5 rounded-full border border-white/10 bg-[#1a1f35]/20 text-white font-semibold cursor-pointer transition hover:brightness-110">
            View Dashboard
          </button>
        </div>

        {/* เส้นคั่น */}
        <hr className="border-white/10 w-full max-w-[300px] hidden" />

        {/* 6) Stats */}
        <div className="flex gap-5 justify-center pb-10">
          <div>
            <h1 className="text-3xl font-bold">{activeIssues}+</h1>
            <p className="text-white/50 text-sm">Active Issues</p>
          </div>

          <div>
            <h1 className="text-3xl font-bold">{resolvedCases}+</h1>
            <p className="text-white/50 text-sm">Resolved Cases</p>
          </div>
        </div>
      </div>

      {/* ================= DESKTOP VERSION (ของเดิม) ================= */}
      <div className="hidden lg:flex justify-between items-start py-20 pt-50">
        {/* LEFT */}
